...
export const ItineraryPane = React.memo(function ItineraryPane({
  ...
  const canInteract = Boolean(tripId);
  ...
  const handleToggleTodoLine = async (line1Based: number) => {
    if (!tripId) return;
    await apiFetch(`/api/trips/${tripId}/itinerary/toggle-todo`, { method: 'POST', body: JSON.stringify({ line: line1Based }) });
    onRefresh();
  };
  ...
  const handleSave = async () => {
    if (!tripId) return;
    setIsSaving(true);
    const res = await apiFetch(`/api/trips/${tripId}/itinerary`, { method: 'PUT', body: JSON.stringify({ content: draft }) });
    setIsSaving(false);
    if (!res.ok) {
  ...
  const MarkdownImage = ({ node, ...props }: any) => {
    ...
    const src = typeof props.src === 'string' ? withApiBase(props.src) : props.src;
          ...
          <div className="prose-terminal itinerary-prose">
            <ReactMarkdown
              ...
              components={{
                ...
                h6: renderHeading(6),
                a: ({ node, ...props }) => {
                  const href = typeof props.href === 'string' ? withApiBase(props.href) : props.href;
