...
import { useAppRouter, buildTravelUrl, type AppPage } from "../hooks/useAppRouter";
...
export type TravelPageProps = {
  ...
  onEvent: (handler: (event: { event: string; payload: any }) => void) => void;
  offEvent: (handler: (event: { event: string; payload: any }) => void) => void;
...
export const TravelPage: React.FC<TravelPageProps> = ({
  ...
  const initialRouteRef = useRef<{ tripId: string | null; conversationId: string | null } | null>({
    tripId: initialTripId ?? null,
  ...
  const { navigateTravel, syncTravelUrl } = useAppRouter(
    useCallback((route: AppPage) => {
      if (route.page === "travel") {
        const tripId = route.tripId ?? null;
        const conversationId = route.conversationId ?? null;
        ...
        setActiveTripId((prevTripId) => {
          ...
          pendingRouteRef.current = tripId ? { tripId, conversationId } : null;
          return tripId;
        });
        setActiveConversationId((prevConversationId) => {
          if (conversationId === prevConversationId) return prevConversationId;
  ...
  // Register/unregister our event handler with the parent AppShell
  useEffect(() => {
    ...
    if (initialRoute && res.data.length > 0) {
      ...
      if (initialRoute.tripId) {
        ...
        if (validTrip) {
          ...
          syncTravelUrl(res.data[0].id, null, res.data[0].name);
        }
      } else {
        setActiveTripId(res.data[0].id);
        syncTravelUrl(res.data[0].id, null, res.data[0].name);
  ...
  const handleSelectTrip = useCallback((tripId: string) => {
    ...
    navigateTravel(tripId, null, { tripName });
    setActiveTripId(tripId);
  }, [activeTripId, navigateTravel, trips]);

  const handleSelectConversation = useCallback((conversationId: string) => {
    if (!activeTripId) return;
    if (conversationId === activeConversationId) return;
    navigateTravel(activeTripId, conversationId, { tripName: activeTripName });
    setActiveConversationId(conversationId);
  }, [activeTripId, activeConversationId, navigateTravel, activeTripName]);
  ...
  useEffect(() => {
    if (!urlSyncEnabledRef.current) return;
    syncTravelUrl(activeTripId, activeConversationId, activeTripName);
  }, [activeTripId, activeConversationId, activeTripName, syncTravelUrl]);
  ...
  const handleNewTripSubmit = async () => {
    ...
    if (convRes.ok) {
      ...
      setMessages([]);
      setIsLoading(false);
      navigateTravel(res.data.id, convRes.data.id, { tripName: res.data.name });
      setActiveTripId(res.data.id);
      setActiveConversationId(convRes.data.id);
    } else {
      navigateTravel(res.data.id, null, { tripName: res.data.name });
      setActiveTripId(res.data.id);
    }
  };
  ...
  const handleDeleteTrip = useCallback(async () => {
    ...
    if (remaining.length > 0) {
      navigateTravel(remaining[0].id, null, { tripName: remaining[0].name });
      setActiveTripId(remaining[0].id);
      return;
    }
    navigateTravel(null, null);
    ...
    runIdToTextRef.current = {};
    runIdToToolActivityRef.current = {};
  }, [activeTripId, activeTrip?.name, trips, navigateTravel]);
  ...
  const handleCreateConversation = async () => {
    ...
    navigateTravel(activeTripId, res.data.id, { tripName: activeTripName });
  ...
  const handleDeleteConversation = async (conversation: Conversation) => {
    ...
    if (conversation.id === activeConversationId) {
      if (remaining.length > 0) {
        navigateTravel(activeTripId, remaining[0].id, { tripName: activeTripName });
        setActiveConversationId(remaining[0].id);
      } else {
        navigateTravel(activeTripId, null, { tripName: activeTripName });
        setActiveConversationId(null);
        setMessages([]);
        setIsLoading(false);
        setConversationProgress({});
  ...
  }, [activeTripId, activeTripName, navigateTravel, sendChatMessage]);
  ...
  }, [activeTripId, handleStartConversationWithPrompt]);
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "hsl(var(--bg-primary))" }}>
      {/* Trip pills bar */}
      <div
        className="travel-trip-bar px-5 py-2"
        style={{
      ...
      >
        ...
        {connectionError && (
          <div className="mt-2 p-2 rounded-lg" style={{ background: "hsl(var(--error) / 0.1)" }}>
      ...
      <nav className="mobile-tab-bar travel-mobile-tab-bar">
